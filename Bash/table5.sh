make ins_sil
make silent INPUT=input_1_svt3_01.txt
cp math_script.wl Scripts/math_script_1_svt3_01.wl
make silent INPUT=input_2_svt3_01.txt
cp math_script.wl Scripts/math_script_2_svt3_01.wl
make silent INPUT=input_3_svt3_01.txt
cp math_script.wl Scripts/math_script_3_svt3_01.wl
make silent INPUT=input_4_svt3_01.txt
cp math_script.wl Scripts/math_script_4_svt3_01.wl
make silent INPUT=input_5_svt3_01.txt
cp math_script.wl Scripts/math_script_5_svt3_01.wl
mv input.cpp input2.cpp
mv input3.cpp input.cpp
make ins_sil
make silent INPUT=input_1_svt3_01.txt
cp math_script.wl Scripts/math_script_1_svt3_01.wl
make silent INPUT=input_2_svt3_01.txt
cp math_script.wl Scripts/math_script_2_svt3_01.wl
make silent INPUT=input_3_svt3_01.txt
cp math_script.wl Scripts/math_script_3_svt3_01.wl
make silent INPUT=input_4_svt3_01.txt
cp math_script.wl Scripts/math_script_4_svt3_01.wl
make silent INPUT=input_5_svt3_01.txt
cp math_script.wl Scripts/math_script_5_svt3_01.wl
mv input.cpp input3.cpp
mv input2.cpp input.cpp
