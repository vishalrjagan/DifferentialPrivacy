...
{
	ofstream out;
	out.open("Inputs/input_"+toString(n)+"_svt7.txt", ios::app);
	out<<"real lhs"<<j<<";"<<endl<<"lhs"<<j<<" = Lap \"eps / ( 4 )\" q"<<j<<";"<<endl;
	...
	if(j<n-1)
	{
		...
		out<<"rhs"<<(k+1)<<" = Lap \"eps / ( 2 )\" zero;"<<endl;
		out.close();
...
int main()
{
	...
	ofstream out;
	out.open("Inputs/input_"+toString(n)+"_svt7.txt", ios::out);
	out<<"-5 5"<<endl;
	for(int i=0;i<n;i++)
