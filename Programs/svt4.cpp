...
int main()
{
	...
	ofstream out;
	out.open("Inputs/input_"+toString(n)+"_svt4.txt", ios::out);
	out<<"-5 5"<<endl;
	for(int i=0;i<n;i++)
	...
	out<<"count = 0;"<<endl;
	out<<"real rhs"<<0<<";"<<endl;
	out<<"rhs"<<0<<" = Lap \"eps / ( 4 * del )\" zero;"<<endl;
	for(int j=0;j<n;j++)
	{
		out<<"real lhs"<<j<<";"<<endl;
		out<<"q"<<j<<" = q"<<j<<" - t"<<j<<";"<<endl;
		out<<"lhs"<<j<<" = Lap \"3 * eps / ( 4 * del )\" q"<<j<<";"<<endl;
		out<<"out"<<j<<" = lhs"<<j<<" >= rhs"<<0<<";"<<endl;
		out<<"if out"<<j<<" then count = count + one; fi;"<<endl;
