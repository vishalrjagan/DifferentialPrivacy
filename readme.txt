***Directory Contents***
	...
	input.cpp: A program which contains one function compare, which defines the adjacency relation.
...
******Summary
	To run from program compilation,
		make level0 result INPUT=<filename> FRAC=<frac> EPS=<eps>
	To run from adjacency relation creation,
		make level1 result INPUT=<filename> FRAC=<frac> EPS=<eps>
	To run from script file creation,
		make level2 result INPUT=<filename> FRAC=<frac> EPS=<eps>
	To only run till script creation, but avoid running script file on mathematica, remove the result option from the above commands, i.e., run the above commands as
		make levelX INPUT=<filename> FRAC=<frac> EPS=<eps>

******Detailed
	make [options] INPUT=<filename> FRAC=<frac> EPS=<eps>
